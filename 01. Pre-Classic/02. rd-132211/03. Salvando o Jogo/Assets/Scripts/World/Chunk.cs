...
public class Chunk : MonoBehaviour {
    ...
    private void Start() {
        ...
        /*
        // Serializa os voxels como JSON
        string json = JsonUtility.ToJson(voxelDataList);

        // Cria um arquivo JSON e grava os voxels nele
        //string fileName = "voxels.json";
        //string filePath = Path.Combine(Application.dataPath, "../Data", fileName);
        string filePath = Application.dataPath + "/voxels.json";

        // Cria um arquivo JSON e grava os voxels nele
        File.WriteAllText(filePath, json);

        // Carrega os voxels do arquivo JSON
        string jsonLoaded = File.ReadAllText(filePath);
        VoxelType[,,] loadedVoxels = JsonUtility.FromJson<VoxelType[,,]>(jsonLoaded);
        */

        string jsonString = JsonUtility.ToJson(voxelDataList);
        File.WriteAllText(Application.dataPath + "/chunk.json", jsonString);
    }
            ...
            for(int y = 0; y < ChunkSizeInVoxels.y; y++) {
                for(int z = 0; z < ChunkSizeInVoxels.z; z++) {
                    if(voxelMap[x, y, z] != VoxelType.air) {
                        ...
                        voxelData.y = y;
                        voxelData.z = z;
                        voxelData.type = voxelMap[x, y, z].ToString();
